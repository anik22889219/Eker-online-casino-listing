...
// Seed system agents in background
if (db) {
  MigrationService.ensureSystemAgentsSeeded(db).catch(err => {
    console.warn("[MigrationService] Seed background error:", err);
  });
}
...
app.post("/api/analyze-game-logo", async (req, res) => {
  ...
  try {
    ...
    if (db) {
      MigrationService.logMigrationEvent(db, {
        feature: "Banner/Promotion Analyzer",
        mappedAgentId: "agent-promo-vision",
        mappedAgentName: "Promo Vision Agent",
        status: "success",
        inputSummary: `Analyzed Game Logo: ${logoUrl}`,
        outputSummary: `Detected Game: ${detectedName} (${detectedBrand})`
      });
      MigrationService.recordAgentRun(db, "agent-promo-vision", "Game Logo Analysis", { logoUrl }, logoResultObj, 800, "completed");
    }

    return res.json(logoResultObj);
  } catch (err: any) {
    ...
    if (db) {
      MigrationService.logMigrationEvent(db, {
        ...
        status: "fallback",
        inputSummary: `Analyzed Game Logo: ${logoUrl}`,
        outputSummary: "Heuristic Fallback Used"
      });
      MigrationService.recordAgentRun(db, "agent-promo-vision", "Game Logo Analysis", { logoUrl }, details, 100, "completed");
    }
    return res.json({
      success: true,
      ...details,
...
app.post("/api/extract-promo", async (req, res) => {
  ...
  if (!apiKey) {
    ...
    const fallbackSlogan = "Exclusive Deposit Bonus";
    if (db) {
      MigrationService.logMigrationEvent(db, {
        feature: "Banner/Promotion Analyzer",
        mappedAgentId: "agent-promo-vision",
        mappedAgentName: "Promo Vision Agent",
        status: "fallback",
        inputSummary: `Extract Promo Slogan: ${bannerImageUrl}`,
        outputSummary: fallbackSlogan
      });
      MigrationService.recordAgentRun(db, "agent-promo-vision", "Banner Slogan Extraction", { bannerImageUrl }, { welcomeSlogan: fallbackSlogan }, 50, "completed");
    }
    return res.json({
      success: true,
      welcomeSlogan: fallbackSlogan
    });
  }
  ...
  try {
    ...
    const welcomeSlogan = output.welcomeSlogan || "Exclusive Deposit Bonus";
    ...
    if (db) {
      MigrationService.logMigrationEvent(db, {
        ...
        status: "success",
        inputSummary: `Extract Promo Slogan: ${bannerImageUrl}`,
        outputSummary: welcomeSlogan
      });
      MigrationService.recordAgentRun(db, "agent-promo-vision", "Banner Slogan Extraction", { bannerImageUrl }, { welcomeSlogan }, 900, "completed");
    }
    ...
    if (db) {
      ...
      MigrationService.recordAgentRun(db, "agent-promo-vision", "Banner Slogan Extraction", { bannerImageUrl }, { welcomeSlogan: fallbackSlogan }, 100, "completed");
    }
    return res.json({
      success: true,
      welcomeSlogan: fallbackSlogan
...
`.trim();
      ...
      if (db) {
        MigrationService.logMigrationEvent(db, {
          feature: "Casino Content Generator",
          mappedAgentId: "agent-casino-content-writer",
          mappedAgentName: "Casino Content Writer Agent",
          status: "success",
          inputSummary: `Link: ${affiliateLink}`,
          outputSummary: `Generated Review for ${geminiOutput.casinoName}`
        });
        MigrationService.recordAgentRun(db, "agent-casino-content-writer", "Casino Review Generation", { affiliateLink, casinoId }, geminiOutput, 1600, "completed");
      }
      ...
      if (db) {
        MigrationService.logMigrationEvent(db, {
          ...
          status: "fallback",
          inputSummary: `Link: ${affiliateLink}`,
          outputSummary: `Free Heuristic Review for ${freeData.casinoName}`
        });
        MigrationService.recordAgentRun(db, "agent-casino-content-writer", "Casino Review Generation", { affiliateLink, casinoId }, freeData, 200, "completed");
      }
...
`.trim();
  ...
  if (!apiKey) {
    ...
    if (db) {
      MigrationService.logMigrationEvent(db, {
        feature: "SEO Functionality",
        mappedAgentId: "agent-seo-specialist",
        mappedAgentName: "SEO Specialist Agent",
        status: "fallback",
        inputSummary: `Blog for ${casinoName}`,
        outputSummary: defaultTitle
      });
      MigrationService.recordAgentRun(db, "agent-seo-specialist", "SEO Blog Article Generation", { casinoName, affiliateLink }, freeRes, 200, "completed");
    }
  ...
  try {
    ...
    if (response.text) {
      ...
      if (db) {
        ...
        MigrationService.recordAgentRun(db, "agent-seo-specialist", "SEO Blog Article Generation", { casinoName, affiliateLink }, blogResult, 1800, "completed");
      }
    ...
    try {
      ...
      if (response.text) {
        ...
        if (db) {
          ...
          MigrationService.recordAgentRun(db, "agent-seo-specialist", "SEO Blog Article Generation", { casinoName, affiliateLink }, blogResult, 2200, "completed");
        }
      ...
      if (db) {
        ...
        MigrationService.recordAgentRun(db, "agent-seo-specialist", "SEO Blog Article Generation", { casinoName, affiliateLink }, fallbackBlog, 200, "completed");
      }
...
`;
    ...
    if (db) {
      ...
      MigrationService.logMigrationEvent(db, {
        feature: "AI Chat",
        mappedAgentId: "agent-ai-operations",
        mappedAgentName: "AI Operations Agent",
        status: "success",
        inputSummary: lastMsgText ? lastMsgText.substring(0, 100) : "AI Chat Request",
        outputSummary: finalResponseText ? finalResponseText.substring(0, 100) : "Response generated"
      });
      MigrationService.recordAgentRun(db, "agent-ai-operations", "AI Operations Chat", { messageCount: messages.length }, { response: finalResponseText }, 1200, "completed");
    }
...
// Centralized Multi-Agent Execution Engine Endpoint
const handleAgentExecution = async (req: express.Request, res: express.Response) => {
  try {
    const result = await AgentExecutorService.executeTask(db, req.body);
    return res.json({
...
app.post("/api/ai-agent/execute", handleAgentExecution);
app.post("/api/ai/agent/execute", handleAgentExecution);

// Skills API Endpoints
app.get("/api/ai-agent/skills", async (req, res) => {
  try {
    if (!db) return res.json({ success: true, skills: [] });
...
app.post("/api/ai-agent/skills", async (req, res) => {
  try {
    ...
    await db.collection("skills").doc(id).set(skillDoc, { merge: true });
    return res.json({ success: true, skill: skillDoc });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
});

app.delete("/api/ai-agent/skills/:id", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
...
app.get("/api/ai-agent/knowledge", async (req, res) => {
  try {
    if (!db) return res.json({ success: true, knowledge: [] });
    const snap = await db.collection("ai-agent-knowledge").get();
    const knowledge = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
...
app.post("/api/ai-agent/knowledge", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const kData = req.body;
    const id = kData.id || crypto.randomUUID();
    const now = new Date().toISOString();
    const kDoc = {
      id,
      title: kData.title || "Untitled Document",
      category: kData.category || "general",
      content: kData.content || "",
      tags: kData.tags || [],
      version: kData.version || 1,
      status: kData.status || "active",
      createdAt: kData.createdAt || now,
      updatedAt: now
    };
    await db.collection("ai-agent-knowledge").doc(id).set(kDoc, { merge: true });
    return res.json({ success: true, document: kDoc });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
});

app.delete("/api/ai-agent/knowledge/:id", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
    await db.collection("ai-agent-knowledge").doc(id).delete();
    return res.json({ success: true, deletedId: id });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
...
// Agent Runs Log Endpoint
app.get("/api/ai-agent/runs", async (req, res) => {
  try {
    if (!db) return res.json({ success: true, runs: [] });
    const agentId = req.query.agentId as string;
    let query: any = db.collection("agentRuns");
    if (agentId) {
      query = query.where("agentId", "==", agentId);
    }
    const snap = await query.limit(50).get();
    const runs = snap.docs.map((doc: any) => ({ id: doc.id, ...doc.data() }));
...
// Migration Audit Logs Endpoint
app.get("/api/migration/logs", async (req, res) => {
  try {
    const logs = await MigrationService.getMigrationLogs(db, 100);
...
app.get("/api/workflows", async (req, res) => {
  try {
    if (!db) return res.json({ success: true, workflows: [] });
    const snap = await db.collection("workflows").get();
    const workflows = snap.docs.map((doc: any) => ({ id: doc.id, ...doc.data() }));
...
app.post("/api/workflows", async (req, res) => {
  try {
    ...
    const id = wData.id || crypto.randomUUID();
    const now = new Date().toISOString();

    const workflowDoc = {
      id,
      name: wData.name || "New Workflow",
      description: wData.description || "",
      category: wData.category || "content_pipeline",
      status: wData.status || "active",
      nodes: wData.nodes || [],
      edges: wData.edges || [],
      createdAt: wData.createdAt || now,
      updatedAt: now
    };

    await db.collection("workflows").doc(id).set(workflowDoc, { merge: true });
    return res.json({ success: true, workflow: workflowDoc });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
});
...
app.post("/api/workflows/:id/start", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
    const { initialInput, triggeredBy } = req.body;

    const run = await WorkflowEngineService.startWorkflow(db, {
      workflowId: id,
...
// 5. Get All Workflow Runs
app.get("/api/workflow-runs", async (req, res) => {
  try {
    if (!db) return res.json({ success: true, runs: [] });
    const workflowId = req.query.workflowId as string;
    let query: any = db.collection("workflowRuns");
    if (workflowId) {
...
// 6. Get Workflow Run Details with Steps
app.get("/api/workflow-runs/:id", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
    const runSnap = await db.collection("workflowRuns").doc(id).get();
    if (!runSnap.exists) {
      return res.status(404).json({ success: false, error: "Workflow run not found" });
    }

    const run = { id: runSnap.id, ...runSnap.data() };
    const stepsSnap = await db.collection("workflowSteps").where("runId", "==", id).get();
    const steps = stepsSnap.docs.map((doc: any) => ({ id: doc.id, ...doc.data() }));

    return res.json({ success: true, run, steps });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
});

// 7. Human Approval Action (Approve / Reject)
app.post("/api/workflow-runs/:id/approve", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
    const { action, notes, approvedBy } = req.body;

    const run = await WorkflowEngineService.resumeApproval(db, {
      runId: id,
...
// 8. Retry Workflow Run Step
app.post("/api/workflow-runs/:id/retry", async (req, res) => {
  try {
    if (!db) return res.status(500).json({ success: false, error: "Database not connected" });
    const { id } = req.params;
    const { nodeId } = req.body;

    const run = await WorkflowEngineService.retryRun(db, id, nodeId);
    return res.json({ success: true, run });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
...
app.post("/api/workflow-runs/:id/cancel", async (req, res) => {
  try {
    ...
    const run = await WorkflowEngineService.cancelRun(db, id, reason);
    return res.json({ success: true, run });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
...
startServer().catch((err) => {
  console.error("[Express] Fatal startup error:", err);
});
