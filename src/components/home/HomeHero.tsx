...
export const HomeHero: React.FC<HomeHeroProps> = ({ config }) => {
  ...
  const gamesList = theme?.mostWinningGames || [];
  ...
  const filteredGameSuggestions = gameName.trim() === ""
    ? gamesList.map(g => g.name).filter(Boolean)
    : gamesList
        .filter(g => (g.name || "").toLowerCase().includes(gameName.toLowerCase()) || (g.brandName || "").toLowerCase().includes(gameName.toLowerCase()))
        .map(g => g.name);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!appName || !jackpotAmount || !dateTime) {
    ...
    try {
      ...
      await addDoc(collection(db, "sellRequests"), {
        ...
        message: `Jackpot submission for ${appName}${gameName ? ` (${gameName})` : ""}. Time: ${dateTime}.`,
        status: "pending",
              ...
              <div className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
                ...
                {!user ? (
                  ...
                  <div className="py-8 text-center space-y-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-slate-400 max-w-sm mx-auto leading-normal font-semibold">
                        ধন্যবাদ! আপনার স্ক্রিনশটটি এডমিন প্যানেলে পাঠানো হয়েছে। যাচাইকরণের পর টাকা পাঠিয়ে দেওয়া হবে। আপনি আপনার প্রোফাইল পেজে এর লাইভ স্ট্যাটাস দেখতে পারবেন।
                      </p>
                    </div>
                    <button
                      onClick={() => {
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4 text-left">
                    ...
                    {/* Game Name Searchable Dropdown */}
                    <div className="space-y-1 relative">
                      <label htmlFor="modal-game-name" className="text-xs font-bold text-slate-400">
                        Game Name (গেমের নাম সিলেক্ট / সার্চ করুন)
                      </label>
                      <input
                        id="modal-game-name"
                        type="text"
                        placeholder="গেমের নাম লিখুন বা সিলেক্ট করুন (যেমন: Aviator)"
                        value={gameName}
                        onChange={(e) => {
                      ...
                      {showGameSuggestions && uniqueGameSuggestions.length > 0 && (
                        <div className="absolute left-0 right-0 z-50 mt-1 max-h-48 overflow-y-auto bg-slate-950 border border-slate-800 rounded-xl shadow-2xl divide-y divide-slate-900 scrollbar-thin scrollbar-thumb-slate-800">
                          {uniqueGameSuggestions.map((gName, index) => {
                            const gameObj = gamesList.find((g) => g.name === gName);
                            return (
                              <button
                                key={gName}
                                type="button"
                                ...
                                className={`w-full text-left px-4 py-2.5 text-xs font-semibold transition-colors duration-150 flex items-center justify-between cursor-pointer ${
                                  index === highlightedGameIndex ? "bg-indigo-600 text-white" : "hover:bg-slate-900 text-slate-300"
                                }`}
                              >
                                <div className="flex items-center gap-2 truncate">
                                  <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${index === highlightedGameIndex ? "bg-white" : "bg-indigo-500"}`} />
                                  <span className="truncate">{gName}</span>
                                </div>
                                {gameObj?.brandName && (
                                  <span className={`text-[9px] px-1.5 py-0.5 rounded font-extrabold uppercase shrink-0 ${index === highlightedGameIndex ? "bg-indigo-700 text-indigo-100" : "bg-slate-800 text-indigo-400"}`}>
                                    {gameObj.brandName}
                                  </span>
                                )}
                              </button>
