...
import { Agent, Skill } from "../../types/firestore";
import { AgentDashboard } from "./agent/AgentDashboard";
import { AgentDrawer } from "./agent/AgentDrawer";
import { AgentPlaygroundModal } from "./agent/AgentPlaygroundModal";
import { AgentDeleteModal } from "./agent/AgentDeleteModal";
import { WorkflowEngineManager } from "./WorkflowEngineManager";
import { SystemDetailsView } from "./agent/SystemDetailsView";

// Initial seed template for system agents
const DEFAULT_INITIAL_AGENTS: Omit<Agent, "id">[] = [
  {
    name: "SEO Listing Specialist",
    role: "Casino & SEO Content Strategist",
    description: "Generates high-converting, search-optimized casino review drafts, focus keywords, meta tags, and structured FAQ schemas.",
    avatar: "search",
    category: "seo",
    status: "active",
    systemInstructions: "You are an expert SEO Specialist for gambling affiliate websites. Your primary goal is to optimize casino listing titles, meta descriptions, focus keywords, and high-converting landing page text while strictly ensuring compliance with search engine guidelines and user intent.",
    shortContext: "Focus on gambling affiliate SEO, high click-through titles, and keyword optimization.",
    personality: "Data-driven, precise, and growth-focused.",
    writingStyle: "Engaging, SEO-optimized, clear headings.",
    ...
    skillIds: ["seo_analysis", "keyword_research"],
  ...
  },
  {
    ...
    description: "Analyzes user-submitted reviews, jackpot win screenshots, and sell requests to detect fraud or spam.",
    avatar: "shield",
    category: "moderation",
    status: "active",
    systemInstructions: "You are an AI Compliance Auditor responsible for reviewing user-submitted reviews, jackpot screenshots, and promotional submissions. Verify that the screenshots are authentic, check for spam or offensive language, and summarize findings clearly.",
    shortContext: "Audit user submissions for fraud, offensive content, or fake winning claims.",
    personality: "Vigilant, fair, and methodical.",
    writingStyle: "Concise, structured evaluation summaries.",
    tone: "Objective and cautious.",
    modelConfig: { modelName: "gemini-2.5-flash", temperature: 0.2, maxTokens: 2048, topP: 0.95 },
    permissions: { canReadData: true, canWriteData: true, canExecuteActions: true, canManageUsers: false },
    skillIds: ["image_ocr", "text_moderation"],
    knowledgeIds: ["moderation_policy"],
    toolIds: [],
    totalRuns: 28,
    successfulRuns: 27,
    failedRuns: 1,
  ...
  },
  {
    ...
    description: "Extracts operator stats, verifies local payment channels (e.g. bKash, Nagad), and compares welcome bonus terms.",
    avatar: "zap",
    category: "casino",
    status: "active",
    systemInstructions: "You are an expert Casino Intelligence Analyst. You generate in-depth casino operator reviews, analyze welcome bonuses, verify payout options (e.g., bKash, Nagad, Crypto), and outline pros & cons objectively.",
    shortContext: "Deep knowledge of online casino operators, payment gateways, and promotion structures.",
    personality: "Analytical, thorough, and trustworthy.",
    writingStyle: "Structured bullet points, tables, and pros/cons.",
    tone: "Neutral and informative.",
    modelConfig: { modelName: "gemini-2.5-pro", temperature: 0.4, maxTokens: 4096, topP: 0.95 },
    permissions: { canReadData: true, canWriteData: true, canExecuteActions: false, canManageUsers: false },
    skillIds: ["casino_scraping", "bonus_evaluator"],
  ...
  },
  {
    ...
    description: "Assists players with deposit bonuses, affiliate link redirects, screenshot submissions, and general platform inquiries.",
    avatar: "support",
    category: "customer_support",
    status: "active",
    systemInstructions: "You are a friendly Customer Support Assistant. Help users navigate casino deals, understand affiliate link rewards, submit win screenshots, and resolve common queries efficiently.",
    shortContext: "Customer assistance for player platform usage.",
    personality: "Warm, empathetic, and direct.",
    writingStyle: "Conversational, step-by-step guidance.",
    tone: "Helpful and welcoming.",
    modelConfig: { modelName: "gemini-2.5-flash", temperature: 0.6, maxTokens: 1024, topP: 0.95 },
    permissions: { canReadData: true, canWriteData: false, canExecuteActions: false, canManageUsers: false },
    skillIds: ["faq_retrieval"],
    knowledgeIds: ["support_faq"],
    toolIds: [],
    totalRuns: 45,
    successfulRuns: 45,
    failedRuns: 0,
...
export const AIAgentManager: React.FC = () => {
  const [activeTab, setActiveTab] = useState<"agents" | "skills" | "workflows" | "migration" | "sysinfo" | "chat" | "memory" | "knowledge" | "notifications" | "settings">("agents");
  ...
  const [newSkillCategory, setNewSkillCategory] = useState<string>("seo");
  ...
  // ==========================================
  useEffect(() => {
    // 0. Subscribe to Agents
    const unsubAgents = onSnapshot(
      query(collection(db, "agents"), orderBy("updatedAt", "desc")),
      (snap) => {
        ...
        if (list.length === 0 && !snap.metadata.hasPendingWrites) {
          handleSeedDefaultAgents();
        }
      },
      (err) => {
        console.warn("Agents subscription info:", err);
        setLoadingAgents(false);
      }
    );

    // 0.1 Subscribe to Skills
    const unsubSkills = onSnapshot(
      query(collection(db, "skills"), orderBy("updatedAt", "desc")),
      (snap) => {
      ...
      (err) => console.warn("Skills subscription info:", err)
    );
    ...
    // 5. Subscribe to Migration Audit Logs & Agent Runs
    const unsubMigrationLogs = onSnapshot(
      query(collection(db, "migration_logs"), orderBy("timestamp", "desc")),
      (snap) => {
        setMigrationLogs(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
      },
      (err) => console.warn("Migration logs subscription info:", err)
    );

    const unsubAgentRuns = onSnapshot(
      query(collection(db, "agentRuns"), orderBy("createdAt", "desc")),
      (snap) => {
        setAgentRuns(snap.docs.map((d) => ({ id: d.id, ...d.data() })));
      },
      (err) => console.warn("Agent runs subscription info:", err)
    );
  ...
  const handleSeedDefaultSkills = async () => {
    try {
      const defaultSkills: Partial<Skill>[] = [
        {
          id: "seo_optimizer",
          name: "SEO Keyword & Meta Optimizer",
          category: "seo",
          description: "Generates rank-ready SEO meta titles, meta descriptions, and keyword density guides for casino listings.",
          instructions: "Analyze target keywords, craft click-enticing titles under 60 chars, meta descriptions under 155 chars, and list top 5 focus keywords.",
          version: 1,
          enabled: true,
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        ...
        {
          id: "review_auditor",
          name: "Screenshot & Review Auditor",
          category: "moderation",
          description: "Audits user jackpot win screenshots and review text for fraud, spam, or quality issues.",
          instructions: "Inspect review ratings, verify screenshot amounts, flag suspicious formatting, and output structured pass/fail audit.",
          version: 1,
          enabled: true,
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
        ...
        {
          id: "casino_evaluator",
          name: "Operator Payout & Bonus Intelligence",
          category: "casino",
          description: "Evaluates online casino bonus terms, rollover requirements, and local payment methods.",
          instructions: "Extract welcome bonus percentage, max reward, wagering multiplier, and list Bangladeshi payment gateways like bKash and Nagad.",
          version: 1,
          enabled: true,
          createdAt: new Date().toISOString(),
          updatedAt: new Date().toISOString()
      ...
      console.error("Error seeding default skills:", err);
    }
  };
  ...
  const handleToggleSkill = async (skill: Skill) => {
    await setDoc(doc(db, "skills", skill.id), { enabled: !skill.enabled, updatedAt: new Date().toISOString() }, { merge: true });
  };
  ...
  const handleSeedDefaultAgents = async () => {
    try {
      ...
      for (const agentTemplate of DEFAULT_INITIAL_AGENTS) {
        const docRef = doc(collection(db, "agents"));
        batch.set(docRef, { ...agentTemplate, id: docRef.id });
      }
      await batch.commit();
    } catch (err) {
  ...
  const handleSaveAgent = async (agentData: Partial<Agent>) => {
    ...
    try {
      ...
      const payload: Agent = {
        ...
        role: agentData.role || "Assistant Specialist",
        description: agentData.description || "",
        avatar: agentData.avatar || "bot",
        category: agentData.category || "general",
        status: agentData.status || "active",
        systemInstructions: agentData.systemInstructions || "",
        shortContext: agentData.shortContext || "",
        longContext: agentData.longContext || "",
        personality: agentData.personality || "",
        writingStyle: agentData.writingStyle || "",
        tone: agentData.tone || "",
        modelConfig: {
          modelName: agentData.modelConfig?.modelName || "gemini-2.5-flash",
          temperature: agentData.modelConfig?.temperature ?? 0.7,
          maxTokens: agentData.modelConfig?.maxTokens ?? 2048,
          topP: agentData.modelConfig?.topP ?? 0.95,
        },
        permissions: {
          canReadData: agentData.permissions?.canReadData ?? true,
          canWriteData: agentData.permissions?.canWriteData ?? false,
          canExecuteActions: agentData.permissions?.canExecuteActions ?? false,
          canManageUsers: agentData.permissions?.canManageUsers ?? false,
        },
        skillIds: agentData.skillIds || [],
        knowledgeIds: agentData.knowledgeIds || [],
  ...
  const handleToggleAgentStatus = async (agent: Agent) => {
    try {
      ...
      await setDoc(doc(db, "agents", agent.id), { status: nextStatus, updatedAt: new Date().toISOString() }, { merge: true });
    } catch (err) {
  ...
  const handleConfirmArchiveAgent = async (agentId: string) => {
    setIsDeletingAgent(true);
    try {
      await setDoc(doc(db, "agents", agentId), { status: "archived", updatedAt: new Date().toISOString() }, { merge: true });
      setIsDeleteModalOpen(false);
      setDeletingAgent(null);
    } catch (err) {
  ...
  const handleCloneAgent = async (agent: Agent) => {
    try {
      ...
      await setDoc(doc(db, "agents", newId), clonedAgent);
    } catch (err) {
      console.error("Error cloning agent:", err);
    }
  };

  const handleOpenAgentChat = async (agent: Agent) => {
...
- blogs: Dynamic blog posts (slug, title, excerpt, content, readTime)`
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto min-h-[700px] bg-slate-50 text-slate-800 antialiased">
        ...
        <div className="flex flex-wrap gap-1 bg-white p-1 rounded-2xl border border-slate-200 shadow-xs">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              activeTab === "agents" ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              activeTab === "skills" ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              activeTab === "workflows" ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              activeTab === "migration" ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 cursor-pointer ${
              activeTab === "sysinfo" ? "bg-slate-900 text-white shadow-xs" : "text-slate-500 hover:text-slate-900 hover:bg-slate-100"
            }`}
          >
      ...
      {/* =======================================================
          TAB PANEL: AGENTS MANAGER
          ======================================================= */}
      {activeTab === "agents" && (
        <AgentDashboard
          agents={agents}
          loading={loadingAgents}
          onOpenAgent={handleOpenAgentChat}
          onEditAgent={(agent) => {
          ...
          onSeedDefaults={handleSeedDefaultAgents}
        />
      )}

      {/* =======================================================
          TAB PANEL: SKILLS REGISTRY
          ======================================================= */}
      {activeTab === "skills" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-4 h-fit">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900">Create Skill</h3>
                <p className="text-[11px] text-slate-500">Reusable AI execution capability</p>
              </div>
            </div>
            ...
            <form onSubmit={handleAddSkill} className="space-y-4 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. SEO Keyword Optimizer"
                  value={newSkillName}
              ...
              <div>
                ...
                >
                  ...
                  <option value="moderation">Moderation & Auditing</option>
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Brief summary of what this skill enables..."
              ...
              <div>
                <label className="block text-slate-700 font-bold mb-1">Skill Execution Instructions *</label>
                <textarea
                  rows={4}
                  required
                  placeholder="Step-by-step directives for the AI model when executing this skill..."
                  value={newSkillInstructions}
                  onChange={(e) => setNewSkillInstructions(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-slate-900 focus:outline-none focus:border-indigo-600 font-mono text-[11px]"
                />
              </div>

              <button
                type="submit"
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl shadow-sm transition flex items-center justify-center gap-2"
              >
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
              <div className="relative flex-1">
                ...
                <input
                  type="text"
                  placeholder="Search skills by name, category, or instructions..."
                  value={searchSkill}
              ...
              <button
                ...
                className="px-3.5 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl flex items-center gap-1.5 transition whitespace-nowrap"
              >
                <RefreshCw className="w-3.5 h-3.5" />
                <span>Reset Default Skills</span>
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {skills
                .filter(
                  (s) =>
                    ...
                    s.instructions.toLowerCase().includes(searchSkill.toLowerCase())
                )
                .map((skill) => (
                  <div
                    key={skill.id}
                    className="bg-white border border-slate-200 rounded-2xl p-4 shadow-sm hover:border-indigo-200 transition space-y-3 flex flex-col justify-between"
                  >
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 text-[10px] font-mono font-bold rounded-md uppercase border border-indigo-100">
                          {skill.category}
                        </span>
                      ...
                      <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-2.5 font-mono text-[10px] text-slate-700 line-clamp-3">
                        {skill.instructions}
                      </div>
                    </div>
              ...
              {skills.length === 0 && (
                <div className="col-span-full bg-white border border-slate-200 rounded-3xl p-8 text-center text-slate-500 space-y-3">
                  <Wrench className="w-8 h-8 text-slate-300 mx-auto" />
                  <p className="text-xs font-bold">No skills registered yet.</p>
                  <button
                    onClick={handleSeedDefaultSkills}
                    className="px-4 py-2 bg-indigo-600 text-white font-bold text-xs rounded-xl hover:bg-indigo-500 transition"
                  >
                    Seed Standard Platform Skills
                  </button>
                </div>
              )}
            </div>
      ...
      {/* =======================================================
          TAB PANEL: WORKFLOW ENGINE
          ======================================================= */}
      {activeTab === "workflows" && (
        <WorkflowEngineManager agents={agents} />
      )}
      ...
      {/* =======================================================
          TAB PANEL: MIGRATION AUDIT & BACKWARD COMPATIBILITY
          ======================================================= */}
      {activeTab === "migration" && (
        <div className="space-y-6">
          ...
          <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 rounded-3xl p-6 text-white shadow-lg space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="space-y-1">
                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-400/30 text-indigo-300 text-xs font-semibold">
                  ...
                  <span>Architecture Migration Status: Active & Verified</span>
                </div>
                <h2 className="text-xl font-black text-white tracking-tight">Multi-Agent AI Migration Audit</h2>
                <p className="text-xs text-slate-300 max-w-2xl">
                  Legacy AI features mapped into specialized Multi-Agent roles. Backward compatibility preserved across all routes with zero data loss or breaking changes.
                </p>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <RefreshCw className="w-3.5 h-3.5" />
                  <span>Re-Seed System Agents</span>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-slate-800 text-xs">
              <div className="bg-slate-800/50 p-3.5 rounded-2xl border border-slate-700/50">
                <span className="text-slate-400 block text-[10px] font-bold uppercase tracking-wider">System Agents</span>
                <span className="text-lg font-black text-emerald-400">4 Active</span>
              </div>
              <div className="bg-slate-800/50 p-3.5 rounded-2xl border border-slate-700/50">
                <span className="text-slate-400 block text-[10px] font-bold uppercase tracking-wider">Legacy Routes Mapped</span>
                <span className="text-lg font-black text-cyan-400">100% Preserved</span>
              </div>
              <div className="bg-slate-800/50 p-3.5 rounded-2xl border border-slate-700/50">
                <span className="text-slate-400 block text-[10px] font-bold uppercase tracking-wider">Migration Audit Logs</span>
                <span className="text-lg font-black text-indigo-300">{migrationLogs.length} Events</span>
              </div>
              <div className="bg-slate-800/50 p-3.5 rounded-2xl border border-slate-700/50">
                <span className="text-slate-400 block text-[10px] font-bold uppercase tracking-wider">Agent Execution Runs</span>
                <span className="text-lg font-black text-purple-300">{agentRuns.length} Runs</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-900">Legacy AI Feature Mapping Matrix</h3>
                  <p className="text-[11px] text-slate-500">Mapping legacy endpoints & state to Multi-Agent Architecture</p>
                </div>
              </div>
              <span className="px-2.5 py-1 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-lg text-[10px] font-bold flex items-center gap-1">
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-slate-100 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                    <th className="py-2.5 px-3">Legacy AI Feature</th>
                    <th className="py-2.5 px-3">Mapped System Agent</th>
                    <th className="py-2.5 px-3">Agent ID</th>
                    <th className="py-2.5 px-3">Status</th>
                    <th className="py-2.5 px-3">Fallback Strategy</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">Casino Content Generator</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 font-semibold">
                        <Bot className="w-3 h-3" />
                        <span>Casino Content Writer Agent</span>
                      </span>
                    </td>
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">agent-casino-content-writer</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Heuristic Content Generator</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">Banner / Promotion Analyzer</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-purple-50 text-purple-700 font-semibold">
                        <Bot className="w-3 h-3" />
                        <span>Promo Vision Agent</span>
                      </span>
                    </td>
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">agent-promo-vision</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Extract Heuristic Game Details</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">SEO Functionality & Blog Generator</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-50 text-emerald-700 font-semibold">
                        <Bot className="w-3 h-3" />
                        <span>SEO Specialist Agent</span>
                      </span>
                    </td>
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">agent-seo-specialist</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Pre-formatted SEO Article Template</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">AI Chat / Operations Assistant</td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">agent-ai-operations</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Direct Gemini Tool Loop</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">AI Memory System</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-semibold">
                        <Brain className="w-3 h-3" />
                        <span>Agent Memory Engine</span>
                      </span>
                    </td>
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">ai-agent-memories</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Firestore Preserved Data</td>
                  </tr>
                  <tr>
                    <td className="py-3 px-3 font-bold text-slate-900">AI Knowledge Base</td>
                    <td className="py-3 px-3">
                      <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 font-semibold">
                    ...
                    <td className="py-3 px-3 font-mono text-[10px] text-slate-500">ai-agent-knowledge</td>
                    <td className="py-3 px-3">
                      <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">Active & Verified</span>
                    </td>
                    <td className="py-3 px-3 text-slate-500">Firestore Preserved Data</td>
          ...
          {/* Audit Event Logs & Execution History Grid */}
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-900">Migration Audit Logs</h3>
                </div>
                <span className="text-[11px] font-semibold text-slate-500">{migrationLogs.length} Records</span>
              </div>

              {migrationLogs.length === 0 ? (
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <Info className="w-6 h-6 text-slate-400 mx-auto mb-2" />
                  <p className="text-xs font-semibold text-slate-600">No migration logs captured yet</p>
                  <p className="text-[11px] text-slate-400 mt-1">Logs will automatically accumulate as legacy routes execute through mapped Agents.</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {migrationLogs.map((log: any) => (
                    <div key={log.id} className="p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-900">{log.feature}</span>
                        <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold ${
                          log.status === "success" ? "bg-emerald-100 text-emerald-800" : log.status === "fallback" ? "bg-amber-100 text-amber-800" : "bg-rose-100 text-rose-800"
                        }`}>
                          {log.status.toUpperCase()}
                        </span>
                      </div>
                      <div className="text-[11px] text-slate-600 flex items-center gap-2">
                        <span className="font-semibold text-indigo-600">{log.mappedAgentName}</span>
                        <span className="text-slate-300">•</span>
                        <span className="font-mono text-[10px] text-slate-400">{new Date(log.timestamp).toLocaleTimeString()}</span>
                      </div>
                      <p className="text-[11px] text-slate-500 truncate">
                        <strong className="text-slate-700">In:</strong> {log.inputSummary}
                      </p>
                      <p className="text-[11px] text-slate-500 truncate">
                        <strong className="text-slate-700">Out:</strong> {log.outputSummary}
                      </p>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-900">Recorded Agent Runs</h3>
                </div>
                <span className="text-[11px] font-semibold text-slate-500">{agentRuns.length} Total Runs</span>
              </div>

              {agentRuns.length === 0 ? (
                <div className="p-8 text-center bg-slate-50 rounded-2xl border border-dashed border-slate-200">
                  <Info className="w-6 h-6 text-slate-400 mx-auto mb-2" />
                  <p className="text-xs font-semibold text-slate-600">No agent run history recorded yet</p>
                  <p className="text-[11px] text-slate-400 mt-1">Runs are stored in Firestore 'agentRuns' collection upon task completion.</p>
                </div>
              ) : (
                <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {agentRuns.map((run: any) => (
                    <div key={run.id} className="p-3 bg-slate-50 rounded-2xl border border-slate-200 text-xs space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-900">{run.taskName || run.agentId}</span>
                        <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md text-[10px] font-bold">
                          {run.status || "COMPLETED"}
                        </span>
                      </div>
                      <div className="text-[11px] text-slate-500 flex items-center gap-2">
                        <span className="font-mono text-[10px] text-indigo-600">{run.agentId}</span>
                        <span className="text-slate-300">•</span>
                        <span>{run.executionTimeMs}ms</span>
                        <span className="text-slate-300">•</span>
                        <span className="font-mono text-[10px] text-slate-400">{new Date(run.createdAt).toLocaleTimeString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {/* =======================================================
          TAB PANEL: SYSTEM DETAILS & ARCHITECTURE
          ======================================================= */}
      {activeTab === "sysinfo" && (
        <SystemDetailsView />
      )}

      {/* AGENT MANAGEMENT MODALS & DRAWERS */}
      <AgentDrawer
      ...
      <AgentDeleteModal
        ...
        onConfirmArchive={handleConfirmArchiveAgent}
